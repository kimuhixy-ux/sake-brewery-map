#!/usr/bin/env python3
"""master_list_raw.jsonの各蔵の住所をNominatim(OSM)でジオコーディングし、
緯度経度を付与してmaster_list_geocoded.jsonに保存する。

Nominatimの利用ポリシーに従い、1秒に1リクエストのレート制限をかける。
番地まで含めた住所でヒットしない場合は、末尾の番地部分を段階的に削って
町・字レベルまで検索範囲を広げる(精度は落ちるが0件よりましと判断)。
どのレベルでヒットしたかを"precision"に記録し、後で目視確認しやすくする。

キャッシュ(住所文字列をキーにした辞書)を都度書き出すため、
途中で中断しても再実行時に再ジオコーディングをスキップできる。

標準ライブラリのみで書かれているので、pip installは不要。
使い方: python3 scripts/geocode_master_list.py
"""
...
INPUT_PATH = BASE_DIR / "master_list_raw.json"
OUTPUT_PATH = BASE_DIR / "master_list_geocoded.json"
CACHE_PATH = BASE_DIR / "master_list_geocode_cache.json"
...
def trim_levels(addr):
    ...
    # 大字・字名まで削っても市区町村レベルの候補が無い場合に備え、
    # 最初に現れる「市/区/町/村」までを切り出した候補を末尾に追加する。
...
def insert_admin_spaces(addr):
    ...
    正しく解析できず0件になることが多いため(例:
    「北海道亀田郡七飯町大中山」は0件だが「北海道 亀田郡 七飯町 大中山」は
    ヒットする)、区切りを明示した候補を優先的に試す。
    """
    chars = []
    for ch in addr:
        chars.append(ch)
