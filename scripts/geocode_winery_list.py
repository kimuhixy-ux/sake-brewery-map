#!/usr/bin/env python3
"""winery_list_raw.jsonの各ワイナリーの住所をNominatim(OSM)でジオコーディングし、
緯度経度を付与してwinery_list_geocoded.jsonに保存する。

ロジックはscripts/geocode_master_list.py・geocode_beer_list.pyと同じ
(住所のクリーニング→番地を段階的に削るフォールバック→行政区画境目への
スペース挿入)。データソースが異なるだけなので、共通化はせずそのまま複製する。

標準ライブラリのみで書かれているので、pip installは不要。
使い方: python3 scripts/geocode_winery_list.py
"""
...
INPUT_PATH = BASE_DIR / "winery_list_raw.json"
OUTPUT_PATH = BASE_DIR / "winery_list_geocoded.json"
CACHE_PATH = BASE_DIR / "winery_list_geocode_cache.json"
