(function () {
  ...
  // 金色(銘柄解説あり)と藍色(通常)のシンプルなピンアイコンをSVGで作る。
  // 画像ファイルを使わずコード内で生成することで、余計なアセット管理を避ける。
  function makeIcon(featured) {
  ...
  const iconNormal = makeIcon(false);
  const iconFeatured = makeIcon(true);
  ...
  function buildPopupHtml(b) {
    ...
    html += `<p class="popup-name">${escapeHtml(b.name)}</p>`;
  ...
  function matchesFilters(b, query, pref) {
  ...
  // 検索・都道府県・トグルの条件に合うものだけをクラスタに出し直す。
  function render() {
    ...
    let count = 0;
    breweries.forEach((b) => {
      if (!matchesFilters(b, query, pref)) return;
      const marker = L.marker([b.lat, b.lon], {
        icon: b.featured ? iconFeatured : iconNormal,
      });
