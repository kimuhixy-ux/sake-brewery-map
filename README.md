...
- ポップアップに蔵名・種別バッジ・代表銘柄・特徴解説・受賞歴（清酒: 全国新酒鑑評会、ワイン: 日本ワインコンクール）・住所・「マップで開く」「経路」（Apple純正マップに連携）・公式サイト/Wikipediaへのリンクを表示
- 現在地ボタン
...
清酒の蔵について、**全国新酒鑑評会**（独立行政法人酒類総合研究所(NRIB)と日本酒造組合中央会が共催する、100年以上続く最も権威のある清酒の公的コンテスト）の入賞歴を、ワイナリーについて**日本ワインコンクール**（OIV(国際ブドウ・ワイン機構)公認、山梨県等が運営する国内最大のワインコンクール）の受賞歴を、地ビールの醸造所について**インターナショナル・ビアカップ(IBC)**（日本地ビール協会(Craft Beer Association)が1996年から毎年開催する国内最大級のビールコンクール）の受賞歴を表示している。「受賞歴のある蔵のみ」トグルで絞り込め、該当するレコードのポップアップにはコンクール名・受賞年（金賞の場合はその旨）が表示される。
...
- 対象は2025年(最新)から2016年までの直近10年分のうち、アーカイブページが公開されている8年度分。2020年度・2021年度はアーカイブページ自体が存在しない(新型コロナ禍でコンクールが中止・非開催になったとみられる)ため欠落する。
- jwine-compe.jpの各年アーカイブページ(https://jwine-compe.jp/archive/{年}/ )が公開する部門別「受賞結果」PDFを情報源とする。掲載されているワインは全て受賞ワインで、金賞には「金」の文字が付く。robots.txtにクロール制限は無いことを確認済み。
- PDFのファイル名・列レイアウトは年度によってばらばら(命名規則が毎年変わる)なため、ファイル名のパターンをハードコードせず、毎年アーカイブページ本体から実際のPDFリンクを発見して取得している。表の抽出も列位置に頼らず、都道府県名・4桁の年・賞名(金/銀/銅)をテキストから直接検出するヒューリスティックで行っている。
- 突合は蔵名の正規化名同士の部分一致(全国新酒鑑評会と同じロジック)で行う。

```bash
python3 scripts/scrape_wine_award_list.py   # 各年のアーカイブページからPDFを取得・解析し、wine_award_raw.jsonを生成
python3 fetch_breweries.py                  # wine_award_raw.jsonをbreweries.jsonに統合
```

- `scrape_wine_award_list.py`も標準ライブラリのみで書かれているが、`pdftotext`コマンド(poppler-utils)が別途必要。
- ワイナリーごとの受賞歴は`breweries.json`の各ワインレコードに、清酒と同じ形の`award`フィールドとして保存される。

### 地ビール: インターナショナル・ビアカップ(IBC)

- 対象は2025年(最新)から2016年までの直近10年分。IBCは1996年から毎年開催されており、この範囲は全て受賞結果PDFが存在する。
- beertaster.org(日本地ビール協会が運営)が公開する各年の受賞結果PDF(`http://beertaster.org/medal/ibc{年}_award.pdf`)を情報源とする。ただし2018年度だけはこの命名規則で別年度(2019年度)の内容が誤って返るため、`ibc2018_award_m.pdf`という別名のファイルを使う。robots.txtにクロール制限は無いことを確認済み。
- PDFは日英併記の受賞リストで、年度によって賞名(GOLD/SILVER/BRONZE、和文では金賞/銀賞/銅賞)の付き方が2パターンある(各行の先頭に直接付く年度と、行の後に独立した行として現れる年度)。列位置やパターンの違いに頼らず、「都道府県名を含む行だけを国内出品者の行として扱う(海外出品者は国名になるため自動的に除外される)」方式で抽出することで、この違いを吸収している。
- 突合は蔵名の正規化名同士の部分一致(全国新酒鑑評会と同じロジック)で行う。

```bash
python3 scripts/scrape_beer_award_list.py   # 各年のPDFを取得・解析し、beer_award_raw.jsonを生成
python3 fetch_breweries.py                  # beer_award_raw.jsonをbreweries.jsonに統合
```

- `scrape_beer_award_list.py`も標準ライブラリのみで書かれているが、`pdftotext`コマンド(poppler-utils)が別途必要。
- 醸造所ごとの受賞歴は`breweries.json`の各地ビールレコードに、清酒と同じ形の`award`フィールドとして保存される。

### 焼酎・泡盛について

焼酎・泡盛には今のところ適用していない。NRIBが本格焼酎・泡盛鑑評会を実施しているが、公開PDFは出品状況・産地別点数などの集計統計のみで、清酒の入賞酒目録のような蔵単位の受賞者リストが無いため、清酒と同じ方式では実装できていない(個別の受賞者リストは県酒造組合単位でしか公開されていない模様)。
...
- データ出典はOpenStreetMapと日本酒造組合中央会「酒蔵検索」、北山産業のクラフトビール醸造所リスト、日本ワイナリー協会の全国ワイナリー一覧、独立行政法人酒類総合研究所(NRIB)の全国新酒鑑評会入賞酒目録、日本ワインコンクールの受賞結果、インターナショナル・ビアカップの受賞結果。網羅性は地域によって差がある。
...
│   ├── scrape_wine_award_list.py
│   └── scrape_beer_award_list.py
└── vendor/
